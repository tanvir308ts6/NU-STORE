    ...
    <v-dialog v-model="dialog" max-width="500px" persistent>
      <v-card>
        <v-card color="primary" flat>
          <v-card-title>
            <span class="headline white--text">Receive Loose Script</span>
            <v-spacer></v-spacer>
            <v-btn
              fab
              class="ma-0 pa-0"
...
export default {
  ...
  watch: {
    dialogStatus(val) {
      this.dialog = val;
      this.$store.commit("SET_LS_COLL_FROM_CENTER_SCHEMA", this.editedData);
  ...
  computed: {
    getSchema() {
      return this.$store.getters.getLsCollFromCenterSchema;
    },
    getItemList() {
  ...
  methods: {
    ...
    save() {
      ...
      this.$store
        .dispatch("saveLsCollFromCenterData", item)
        .then(function(data) {
          try {
            self.close();
    ...
    edit() {
      ...
      let item = this.getSchema;
      this.$store
        .dispatch("editLsCollFromCenterData", item)
        .then(function(data) {
          self.close();
          self.$store.dispatch("fetchLsCollFromCenterData");
        })
