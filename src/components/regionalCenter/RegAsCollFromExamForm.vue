...
export default {
  ...
  watch: {
    dialogStatus(val) {
      this.dialog = val;
      this.$store.commit("SET_REG_AS_COLL_FROM_EXAM_SCHEMA", this.editedData);
  ...
  computed: {
    getSchema() {
      return this.$store.getters.getRegAsCollFromExamSchema;
    },
    getItemList() {
      return this.$store.getters.getRegAsCollFromExamDataList;
    },
    buttonTitle() {
    ...
    formTitle() {
      return this.indexEdited === -1
        ? "Create New Ans Collection From Examiner"
        : "Receive Ans Collection From Examiner";
    }
  },
  methods: {
    validationForm() {
    ...
    save() {
      ...
      this.$store
        .dispatch("saveRegAsCollFromExamData", item)
        .then(function() {
          try {
            self.close();
    ...
    edit() {
      ...
      let item = this.getSchema;
      this.$store
        .dispatch("editRegAsCollFromExamData", item)
        .then(function() {
          self.close();
          self.searchAnswerScriptList();
        })
    ...
    clear() {
      ...
      this.$store.commit("UNSET_REG_AS_COLL_FROM_EXAM_SCHEMA");
