          ...
          <v-card flat class="pr-3 scroll">
            <v-form>
              <v-layout row wrap>
                ...
                <v-flex xs12 v-if="regCenterIdStatus">
                  <v-autocomplete
...
export default {
  ...
  watch: {
    dialogStatus(val) {
      ...
      if (userType == 1) {
        this.regCenterIdStatus = true;
      }
      this.$store.commit("SET_REG_AS_DIST_TO_EXAM_SCHEMA", this.editedData);
  ...
  computed: {
    getSchema() {
      return this.$store.getters.getRegAsDistToExamSchema;
    },
    getItemList() {
    ...
    formTitle() {
      return this.indexEdited === -1
        ? "Create New Ans Delivery To Examiner"
        : "Edit Ans Delivery To Examiner";
    }
  },
  methods: {
    validationForm() {
    ...
    edit() {
      ...
      let item = this.getSchema;
      this.$store
        .dispatch("editRegAsDistToExamData", item)
        .then(function() {
          self.close();
          self.searchAnswerScriptList();
        })
    ...
    clear() {
      ...
      this.$store.commit("UNSET_REG_AS_DIST_TO_EXAM_SCHEMA");
