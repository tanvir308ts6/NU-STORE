...
export default {
  ...
  watch: {
    dialogStatus(val) {
      ...
      this.$store.commit("SET_REG_AS_DIST_TO_NU_FINAL_SCHEMA", this.editedData);
      //this.regionalCenters = this.$store.getters.getCenterItems;
  ...
  computed: {
    getSchema() {
      return this.$store.getters.getRegAsDistToNuFinalSchema;
    },
    getItemList() {
      return this.$store.getters.getRegAsDistToNuFinalDataList;
    },
    buttonTitle() {
    ...
    formTitle() {
      return this.indexEdited === -1
        ? "Create New Ans Delivery To NU Final"
        : "Edit Ans Delivery To NU Final";
    }
  },
  methods: {
    validationForm() {
    ...
    save() {
      ...
      this.$store
        .dispatch("saveRegAsDistToNuFinalData", item)
        .then(function() {
          try {
            self.close();
    ...
    edit() {
      ...
      let item = this.getSchema;
      this.$store
        .dispatch("editRegAsDistToNuFinalData", item)
        .then(function() {
          self.close();
          self.searchAnswerScriptList();
        })
    ...
    clear() {
      ...
      this.$store.commit("UNSET_REG_AS_DIST_TO_NU_FINAL_SCHEMA");
