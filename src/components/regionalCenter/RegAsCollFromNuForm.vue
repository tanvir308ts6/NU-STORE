...
export default {
  ...
  watch: {
    dialogStatus(val) {
      this.dialog = val;
      this.$store.commit("SET_REG_AS_COLL_FROM_NU_SCHEMA", this.editedData);
    },
    dialog(val) {
      if (!val) this.close();
    }
  ...
  computed: {
    getSchema() {
      return this.$store.getters.getRegAsCollFromNuSchema;
    },
    getItemList() {
    ...
    formTitle() {
      return this.indexEdited === -1
        ? "Create New Ans Collection From NU"
        : "Receive Ans Collection From NU";
    }
  },
  methods: {
    validationForm() {
    ...
    save() {
      ...
      this.$store
        .dispatch("saveRegAsCollFromNuData", item)
        .then(function() {
          try {
            self.close();
    ...
    edit() {
      ...
      let item = this.getSchema;
      this.$store
        .dispatch("editRegAsCollFromNuData", item)
        .then(function() {
          self.close();
          self.searchAnswerScriptList();
        })
    ...
    clear() {
      ...
      this.$store.commit("UNSET_REG_AS_COLL_FROM_NU_SCHEMA");
