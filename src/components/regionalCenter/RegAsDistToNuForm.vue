...
export default {
  ...
  watch: {
    dialogStatus(val) {
      ...
      if (userType == 1) {
        this.regcenStatus = true;
      }
      this.dialog = val;
      this.$store.commit("SET_REG_AS_DIST_TO_NU_SCHEMA", this.editedData);
    },
    dialog(val) {
      if (!val) this.close();
    }
  ...
  computed: {
    getSchema() {
      return this.$store.getters.getRegAsDistToNuSchema;
    },
    getItemList() {
      return this.$store.getters.getRegAsDistToNuDataList;
    },
    buttonTitle() {
    ...
    formTitle() {
      return this.indexEdited === -1
        ? "Create New Ans Delivery To NU"
        : "Edit Ans Delivery To NU";
    }
  },
  methods: {
    validationForm() {
    ...
    save() {
      ...
      console.log("save function", item);
      this.$store
        .dispatch("saveRegAsDistToNuData", item)
        .then(function() {
          try {
            self.close();
    ...
    edit() {
      ...
      let item = this.getSchema;
      this.$store
        .dispatch("editRegAsDistToNuData", item)
        .then(function() {
          self.close();
          self.searchAnswerScriptList();
        })
    ...
    clear() {
      ...
      this.$store.commit("UNSET_REG_AS_DIST_TO_NU_SCHEMA");
