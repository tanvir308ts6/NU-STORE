      ...
      <v-card-text>
        <v-form>
          <v-text-field
            ...
            v-validate="'required'"
            :error-messages="errors.collect('college_type')"
            data-vv-name="college_type"
            required
            prepend-icon="edit"
          ></v-text-field>
          <v-checkbox
...
export default {
  ...
  watch: {
    dialogStatus(val) {
      this.dialog = val;
      this.$store.commit("COLLEGE_TYPES_FORM_DATA", this.editedData);
    },
    dialog(val) {
      if (!val) this.close();
    }
  ...
  computed: {
    getSchema() {
      return this.$store.getters.setCollegeType;
    },
    buttonTitle() {
  ...
  methods: {
    ...
    save() {
      ...
      this.$store
        .dispatch("saveCollegeType", item)
        .then(function(data) {
          try {
            self.close();
            self.$store.dispatch("fetchCollegeTypeList");
          } catch (ex) {
            console.log(ex);
          }
        })
    ...
    edit() {
      ...
      let item = this.getSchema;
      this.$store
        .dispatch("updateCollegeType", item)
        .then(function(data) {
          self.close();
          self.$store.dispatch("fetchCollegeTypeList");
        })
