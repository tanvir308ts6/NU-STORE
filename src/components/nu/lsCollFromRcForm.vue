      ...
      <v-card>
        ...
        <v-card-actions>
          <v-layout justify-center>
            <v-btn
              :disabled="editedData.status == 0 ? false : true"
...
export default {
  ...
  data: () => ({
    ...
    dictionary: {
      custom: {
        ...
        comment: {
          required: () => "Select Exam.",
          max: "The Comment field may not be greater than 100 characters"
        }
      }
    }
  }),
  ...
  watch: {
    dialogStatus(val) {
      this.dialog = val;
      this.$store.commit("SET_NU_LS_COLL_FROM_REG_SCHEMA", this.editedData);
  ...
  computed: {
    getSchema() {
      return this.$store.getters.getNuLsCollFromRCSchema;
    },
    // getItemList() {
    ...
    formTitle() {
      return this.indexEdited === -1
        ? "New LS Collection from RC"
        : "Edit LS Collection from RC";
    }
  },
  methods: {
    validationForm() {
    ...
    save() {
      ...
      this.$store
        .dispatch("saveNuLsCollFromRCData", item)
        .then(function(data) {
          try {
            self.close();
            self.$emit("update:loadData", true);
            //self.$store.dispatch("fetchNuLsCollFromRCData");
          } catch (ex) {
            console.log(ex);
          }
        })
    ...
    edit() {
      ...
      let item = this.getSchema;
      this.$store
        .dispatch("editNuLsCollFromRCData", item)
        .then(function(data) {
          self.close();
          self.$emit("update:loadData", true);
          //self.$store.dispatch("fetchNuLsCollFromRCData");
    ...
    clear() {
      ...
      this.$store.commit("UNSET_NU_LS_COLL_FROM_REG_SCHEMA");
