...
export default {
  ...
  computed: {
    getSchema() {
      return this.$store.getters.getNuAsCollFromRegFinalSchema;
    },
    getItemList() {
    ...
    formTitle() {
      return this.indexEdited === -1
        ? "Create New Ans Collection From Reg Final"
        : "Receive Ans Collection From Reg Final";
    }
  },
  methods: {
    validationForm() {
    ...
    save() {
      ...
      this.$store
        .dispatch("saveNuAsCollFromRegFinalData", item)
        .then(function() {
          try {
            self.close();
    ...
    edit() {
      ...
      let item = this.getSchema;
      this.$store
        .dispatch("editNuAsCollFromRegFinalData", item)
        .then(function() {
          self.close();
          self.searchAnswerScriptList();
        })
    ...
    clear() {
      ...
      this.$store.commit("UNSET_NU_AS_COLL_FROM_REG_FINAL_SCHEMA");
