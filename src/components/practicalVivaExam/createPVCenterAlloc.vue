  ...
  <v-container fluid>
    ...
    <v-card v-if="publishState == 1">
      <v-alert :value="true" type="success">
        ...
        <snap class="display-1">Practical/Viva Center List Published</snap>
      </v-alert>
    </v-card>
     <div class="mb-2 pb-2 mt-2">
      <v-btn
        absolute
        right
        color="primary"
    ...
    <div>
      <div class="mx-2 pa-5">
        ...
        <div class="mb-2 pb-2 ml-4">
          ...
          >
            <v-icon dark left>edit</v-icon>Create Auto Center
          </v-btn>
        </div>
        <!-- add  -->
              ...
              <v-card dark class="px-3 CSP_BoarderStyle">
                <v-layout row wrap align-center class="py-3">
                  <!-- <v-autocomplete
                    ...
                    prepend-icon="subject"
                    v-validate="'required'"
                    :error-messages="errors.collect('form1.subject_code')"
                  ...
                  <v-autocomplete
                    ...
                    required
                    :loading="ifFetchingCenterListDone"
                    ...
                    <template v-slot:selection="data">
                      ...
                      >
                        ...
                        <v-btn color="success" class="ma-2" dark
                          >Examinee {{ data.item.student_count }}</v-btn
                        >
                        <v-btn
                          color="primary"
                          class="ma-2"
        ...
        >
          <template v-slot:items="props">
            <!-- <td class="text-xs-left CATCSS_l">
            ...
            <td class="text-xs-left CATCSS">
              ...
              <v-chip label color="indigo" text-color="white">
                <v-avatar> <v-icon>account_circle</v-icon> </v-avatar>Capacity :
                ...
                <template v-slot:items="c_props">
                  ...
                  <td class="text-xs-left CTCSS_r">
                    ...
                    >
                      <h4>
                        [{{ cus.degree }} ]
                        {{ JSON.stringify(cus.subject).split('"').join(" ") }}
                      </h4>
                    </div>
                  </td>
            ...
            >
              ...
              <v-chip
                ...
                label
              >
                <v-avatar> <v-icon>people</v-icon> </v-avatar>Total Students :
...
export default {
  components: { centerAllocationPdf, centerAllocationAuto },
  mixins: [commonGet, common_login_getters],
  props: ["centerAllocation", "submitStatus1", "searchParam","editDataMode",],
  $_veeValidate: {
    validator: "new",
  },
  data() {
    return {
      ...
      ifFetchingCenterListDone: false,
      noDataText: "",
      degreeDataList: [],
      subjectDataList: [],
      fav: true,
    ...
    dialog(val) {
       ...
       if (val) {
        if (this.editDataMode) {
          this.divisionChange(this.getCenterAllocationSchema.division_id);
        }
      }
      // unalloc center
      this.$store
  ...
  created() {
    ...
    if (!this.$store.getters.getCollegeItems.length) {
      // console.log("----> ");
      this.$store.dispatch("fetchCollegeWithStudentCount", param);
    }
    //district
    if (!this.$store.getters.getDistrictItems.length) {
      // console.log("----> ");
      this.$store.dispatch("fetchDistrictData",param);
    }
    //year
  ...
  methods: {
    ...
    removeCollege(item) {
      var position = -1;
      console.log("..", item);
      this.selectedCollege.forEach(function (e, index) {
        if (item.college_code == e.college_code) position = index;
      });
      if (position > -1) this.selectedCollege.splice(position, 1);
    },
    showCollDetails() {
    ...
    getCollgeCode() {
      var param = {
        ...
        division_id: this.getCenterAllocationSchema.division_id,
      ...
      this.noDataText = "Please wait, We are fetching data";
      this.$store
    ...
    divisionChange(val) {
      ...
      var self = this;
      this.searchParam.div_code = val;
      this.searchParam.limit = -1;
      // load center selected division-wise
      this.ifFetchingCenterListDone = true;
      this.$store.dispatch("callCenterData", this.searchParam).then((data) => {
        console.log("ten cen", data);
        if (data) {
          self.ifFetchingCenterListDone = false;
        }
      });
    },
    subjectChange(val) {
      ...
      var dat = this.getSubByVivaPrac.forEach(function (item) {
        if (item.subject_code == val) {
          self.selected_subject_name = item.subject_name;
          console.log("ON subject Changes item ", item);
          return item;
        }
      });
    },
    ...
    addDetails() {
      ...
      this.getCenterAllocationSchema.center.push({
        ...
        //subject_code: this.subject_code,
        subject_name: this.selected_subject_name,
    ...
    editItem(item, index) {
      ...
      this.selected_subject_name = item.subject_name;
    ...
    close() {
      ...
      this.$emit("update:editDataMode", false);
