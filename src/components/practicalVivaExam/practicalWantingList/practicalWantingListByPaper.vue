  ...
  <v-container fluid>
    <v-dialog v-model="pracDialogByStd" fullscreen>
      <v-toolbar dark color="light-blue darken-3" class="mt-4">
        <v-toolbar-title>Practical Wanting List By Examinee</v-toolbar-title>
        <v-spacer></v-spacer>
        <v-toolbar-items>
    ...
    <v-container fluid>
        ...
        <v-layout row>
          ...
          <v-layout justify-center row fill-height align-center>
            <v-btn flat round large>
              Flow: ( {{ getPracticalWantListByPaper_current.center_name }}
              <v-icon dark class="mx-2">arrow_right_alt</v-icon>
              {{ getPracticalWantListByPaper_current.college_name }}
              <v-icon dark class="mx-2">arrow_right_alt</v-icon>
              {{ getPracticalWantListByPaper_current.subject_name }})
            </v-btn>
          </v-layout>
        </v-layout>
      </v-card>
      <v-data-table
        class="elevation-1"
        :headers="headers"
        :items="getPracticalWantListByPaper"
        :loading="loading"
    ...
    <!-- <pre> {{getPracticalWantListByPaper}} </pre> -->
    <!--  <pre>{{searchParam}} </pre>
    <pre>{{getPracticalWantListByPaper_current}} </pre>-->
  </v-container>
</template>
<script>
...
import practicalWantingListByStd from "@/components/practicalVivaExam/practicalWantingList/practicalWantingListByStd.vue";
export default {
  ...
  computed: {
    //all data
    getPracticalWantListByPaper() {
      return this.$store.getters.getPracticalWantListByPaper;
    },
    ...
    getPracticalWantListByPaper_current() {
      return this.$store.getters.getPracticalWantListByPaper_current;
    },
  },
  methods: {
    openDialogueByStd(currentItem) {
      // this.$store.commit("UNSET_PRACTICAL_WANT_DATA_BY_STD");
      this.$store.commit("SET_PRACTICAL_WANT_DATA_BY_STD_CURRENT", currentItem);
    ...
    searchPracticalWantList() {
      ...
      this.setLimit();
      this.$store
        .dispatch("searchPracticalWantDataByPaper", this.searchParam)
        .then((data) => {
          this.loading = false;
  ...
  watch: {
    ...
    getPracticalWantListByPaper_current(val) {
