<template>
  <v-container fluid>
    <v-dialog v-model="pracDialogBySubject" fullscreen>
      <v-toolbar dark color="light-blue darken-2" class="mt-2">
        <v-toolbar-title>Practical Wanting List By Subject</v-toolbar-title>
        <v-spacer></v-spacer>
        <v-toolbar-items>
    ...
    <v-container fluid>
        ...
        <v-layout row>
          ...
          <v-layout justify-center row fill-height align-center>
            <v-btn flat round large
              >Flow: (
              {{ getPracticalWantListByCenter_current.center_name }} )</v-btn
            >
          </v-layout>
        </v-layout>
      </v-card>
      <v-data-table
        class="elevation-1"
        :headers="headers"
        :items="getPracticalWantListByCollege"
        :loading="loading"
    ...
    <!-- <pre>{{getPracticalWantListByCollege}} </pre>
    <pre>{{searchParam}} </pre>
    <pre>{{getPracticalWantListByCenter_current}} </pre>-->
  </v-container>
</template>
...
import practicalWantingListByPaper from "@/components/practicalVivaExam/practicalWantingList/practicalWantingListBySubject.vue";
export default {
  ...
  computed: {
    //all data
    getPracticalWantListByCollege() {
      return this.$store.getters.getPracticalWantListByCollege;
    },
    ...
    getPracticalWantListByCenter_current() {
      return this.$store.getters.getPracticalWantListByCenter_current;
    },
  },
  methods: {
    openDialogueBySubject(currentItem) {
      // this.$store.commit("UNSET_PRACTICAL_WANT_DATA_BY_SUBJECT");
      this.$store.commit(
        "SET_PRACTICAL_WANT_DATA_BY_SUBJECT_CURRENT",
        currentItem
      );
      this.pracDialogBySubject = true;
    },
    closeDialogueByPaper() {
      this.pracDialogBySubject = false;
    },
    ...
    searchPracticalWantList() {
      ...
      this.setLimit();
      this.$store
        .dispatch("searchPracticalWantDataByCollege", this.searchParam)
        .then((data) => {
          this.loading = false;
  ...
  watch: {
    ...
    pracDialogByCol(val) {
      if (val) this.searchPracticalWantList();
    },
    getPracticalWantListByCenter_current(val) {
