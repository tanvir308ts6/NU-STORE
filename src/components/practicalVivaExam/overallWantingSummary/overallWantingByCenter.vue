<template>
  <v-container fluid>
    <v-dialog v-model="overallDialogByCol" fullscreen>
      <v-toolbar dark color="light-blue darken-1">
        <v-toolbar-title>Overall wanting List Summary By College</v-toolbar-title>
        <v-spacer></v-spacer>
        <v-toolbar-items>
      ...
      <v-card style="background-color: white;">
        <overallWantingListByCollege :overallDialogByCol.sync="overallDialogByCol" />
    ...
    <v-container fluid>
        ...
        <v-layout row>
          <v-card flat>
            <v-badge left overlap>
              ...
              <v-card-text class="title">Overall wanting List summary By Center</v-card-text>
            </v-badge>
          </v-card>
      ...
      <v-data-table
        class="elevation-1"
        :headers="headers"
        :items="getOverallWantListByCenter"
        :loading="loading"
      ...
      >
        <template v-slot:items="props">
          ...
          <td class="text-xs">{{ props.item.total_attendance }}</td>
...
import overallWantingListByCollege from "@/components/practicalVivaExam/overallWantingSummary/overallWantingListByCollege.vue";
// import studentDetail from "@/components/examinee/practicalWantingListByCollege.vue";
export default {
  mixins: [commonGet, commonExamYear, common_login_getters],
  components: {
    overallWantingListByCollege,
  },
  data() {
    return {
      overallDialogByCol: false,
  ...
  computed: {
    //all data
    getOverallWantListByCenter() {
      return this.$store.getters.getOverallWantListByCenter;
    },
  ...
  methods: {
    openDialogueByCollege(currentItem) {
      // this.$store.commit("UNSET_OVERALL_WANT_DATA_BY_COLLEGE");
      this.$store.commit(
        "SET_OVERALL_WANT_DATA_BY_CENTER_CURRENT",
        currentItem
      );
      this.overallDialogByCol = true;
    },
    closeDialogueByCollege() {
      this.overallDialogByCol = false;
    },
    ...
    searchOverallWantList() {
      ...
      this.setLimit();
      this.$store
        .dispatch("searchOverallWantDataByCenter", this.searchParam)
        .then((data) => {
          this.loading = false;
