<template>
  <v-container fluid>
    <v-dialog v-model="overallDialogBySubject" fullscreen>
      <v-toolbar dark color="light-blue darken-2" class="mt-2">
        <v-toolbar-title>Overall Wanting List By Subject</v-toolbar-title>
        <v-spacer></v-spacer>
        <v-toolbar-items>
          <v-btn dark flat @click="closeDialogueByPaper()">Close</v-btn>
        </v-toolbar-items>
      </v-toolbar>
      <v-card style="background-color: white;">
        <overallWantingListBySubject />
    ...
    <v-container fluid>
        ...
        <v-layout row>
          <v-card flat>
            <v-badge left overlap>
              ...
              <v-card-text class="title">Overall Wanting List By College</v-card-text>
            </v-badge>
          </v-card>
          <v-divider class="my-2" vertical></v-divider>
          <v-spacer></v-spacer>
          <v-layout justify-center row fill-height align-center>
            <v-btn flat round large>Flow: ( {{getOverallwantlistByCenter_Current.center_name}} )</v-btn>
          </v-layout>
        </v-layout>
      </v-card>
      <v-data-table
        class="elevation-1"
        :headers="headers"
        :items="getOverallWantListByCollege"
        :loading="loading"
      ...
      >
        <template v-slot:items="props">
          ...
          <td class="text-xs">
            <!-- <v-btn @click="openDialogueByPaper()"> -->
            <span>{{ props.item.center_code }} - {{ props.item.center_name }}</span>
            <!-- </v-btn> -->
          </td>
          <td class="text-xs">
            <v-btn @click="openDialogueBySubject(props.item)">
              <span>{{ props.item.college_code }} - {{ props.item.college_name }}</span>
            </v-btn>
          </td>
          <td class="text-xs">
    ...
    <!-- <pre>{{getOverallWantListByCollege}} </pre>
    <pre>{{searchParam}} </pre>
    <pre>{{getOverallwantlistByCenter_Current}} </pre>-->
  </v-container>
</template>
...
import overallWantingListBySubject from "@/components/practicalVivaExam/overallWantingSummary/overallWantingBySubject.vue";
export default {
  ...
  computed: {
    //all data
    getOverallWantListByCollege() {
      return this.$store.getters.getOverallWantListByCollege;
    },
    ...
    getOverallwantlistByCenter_Current() {
      return this.$store.getters.getOverallwantlistByCenter_Current;
    },
  },
  methods: {
    openDialogueBySubject(currentItem) {
      //this.$store.commit("UNSET_OVERALL_WANT_DATA_BY_SUBJECT");
      this.$store.commit(
        "SET_OVERALL_WANT_DATA_BY_SUBJECT_CURRENT",
        currentItem
      );
      this.overallDialogBySubject = true;
    },
    closeDialogueByPaper() {
      this.overallDialogBySubject = false;
    },
    ...
    searchOverallWantList() {
      ...
      this.setLimit();
      this.$store
        .dispatch("searchOverallWantDataByCollege", this.searchParam)
        .then((data) => {
          this.loading = false;
  ...
  watch: {
    ...
    getOverallwantlistByCenter_Current(val) {
