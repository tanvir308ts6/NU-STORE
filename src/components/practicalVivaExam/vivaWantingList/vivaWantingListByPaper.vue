  ...
  <v-container fluid>
    <v-dialog v-model="vivaDialogByStd" fullscreen>
      <v-toolbar dark color="light-blue darken-3" class="mt-4">
        <v-toolbar-title>Viva Wanting List By Examinee</v-toolbar-title>
        <v-spacer></v-spacer>
        <v-toolbar-items>
        ...
        <v-layout row>
          ...
          <v-layout justify-center row fill-height align-center>
            <v-btn flat round large>
              Flow: ( {{ getVivaWantListByPaper_current.center_name }}
              <v-icon dark class="mx-2">arrow_right_alt</v-icon>
              {{ getVivaWantListByPaper_current.college_name }}
              <v-icon dark class="mx-2">arrow_right_alt</v-icon>
              {{ getVivaWantListByPaper_current.subject_name }} )
            </v-btn>
          </v-layout>
        </v-layout>
      </v-card>
    ...
    <!-- <pre> {{getVivaWantListByPaper}} </pre>
    <pre>{{searchParam}} </pre>
    <pre>{{getVivaWantListByPaper_current}} </pre>-->
  </v-container>
</template>
...
import vivaWantingListByStd from "@/components/practicalVivaExam/vivaWantingList/vivaWantingListByStd.vue";
export default {
  ...
  computed: {
    //all data
    getVivaWantListByPaper() {
      return this.$store.getters.getVivaWantListByPaper;
    },
    ...
    getVivaWantListByPaper_current() {
      return this.$store.getters.getVivaWantListByPaper_current;
    },
  },
  methods: {
    openDialogueByStd(currentItem) {
      // this.$store.commit("UNSET_VIVA_WANT_DATA_BY_STD");
      this.$store.commit("SET_VIVA_WANT_DATA_BY_STD_CURRENT", currentItem);
    ...
    searchVivaWantList() {
      ...
      this.setLimit();
      this.$store
        .dispatch("searchVivaWantDataByPaper", this.searchParam)
        .then((data) => {
          this.loading = false;
  ...
  watch: {
    ...
    getVivaWantListByPaper_current(val) {
