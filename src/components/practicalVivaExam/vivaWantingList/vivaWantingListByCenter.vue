<template>
  <v-container fluid>
    <v-dialog v-model="vivaDialogByCol" fullscreen>
      <v-toolbar dark color="light-blue darken-1">
        <v-toolbar-title>Viva Wanting List By College</v-toolbar-title>
        <v-spacer></v-spacer>
        <v-toolbar-items>
      ...
      <v-card style="background-color: white;">
        <vivaWantingListByCollege :vivaDialogByCol.sync="vivaDialogByCol" />
        <!-- <vivaticalWantingListByCollege :dialogStatus.sync="admitCardDialogStatus" :current="current"></admitCardCountByCollege> -->
      </v-card>
    </v-dialog>
...
import vivaWantingListByCollege from "@/components/practicalVivaExam/vivaWantingList/vivaWantingListByCollege.vue";

export default {
  mixins: [commonGet, commonExamYear, common_login_getters],
  components: {
    vivaWantingListByCollege,
  },
  data() {
    return {
      vivaDialogByCol: false,
  ...
  computed: {
    //all data
    getVivaWantListByCenter() {
      return this.$store.getters.getVivaWantListByCenter;
    },
  ...
  methods: {
    openDialogueByCollege(currentItem) {
      console.log("hi mitu");
      this.$store.commit("UNSET_VIVA_WANT_DATA_BY_COLLEGE");
      this.$store.commit("SET_VIVA_WANT_DATA_BY_CENTER_CURRENT", currentItem);
      this.vivaDialogByCol = true;
    },
    closeDialogueByCollege() {
      this.vivaDialogByCol = false;
    },
    ...
    searchVivaWantList() {
      ...
      this.setLimit();
      this.$store
        .dispatch("searchVivaWantDataByCenter", this.searchParam)
        .then((data) => {
          this.loading = false;
