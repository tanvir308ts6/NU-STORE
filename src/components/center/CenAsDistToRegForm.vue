...
export default {
  ...
  watch: {
    dialogStatus(val) {
      ...
      this.$store.commit("SET_CEN_AS_DIST_TO_REG_SCHEMA", this.editedData);
      this.center_code = this.editedData.center_code;
  ...
  methods: {
    ...
    save() {
      ...
      if (user_type == "Center") {
        this.getSchema.center_code = this.getCenterCode();
      } else if (user_type == "Admin") {
        this.getSchema.center_code = this.center_code;
      }
      ...
      this.$store
        .dispatch("saveCenAsDistToRegData", item)
        .then(function() {
          try {
            self.close();
    ...
    edit() {
      ...
      let item = this.getSchema;
      this.$store
        .dispatch("editCenAsDistToRegData", item)
        .then(function() {
          self.close();
          self.searchAnswerScriptList();
        })
    ...
    clear() {
      ...
      this.$store.commit("UNSET_CEN_AS_DIST_TO_REG_SCHEMA");
