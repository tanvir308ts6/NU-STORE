          ...
          <v-card flat class="pr-3 scroll">
            <v-form>
              <v-text-field
                name="division_name"
                label="Division Name"
                v-model="getSchema.division_name"
                v-validate="'required'"
                :error-messages="errors.collect('division_name')"
                data-vv-name="division_name"
                required
                prepend-icon="edit"
              ></v-text-field>
              <v-text-field
...
export default {
  ...
  watch: {
    dialogStatus(val) {
      this.dialog = val;
      this.$store.commit("SET_DIVISION_SCHEMA", this.editedData);
    },
    dialog(val) {
      if (!val) this.close();
    }
  ...
  computed: {
    getSchema() {
      return this.$store.getters.getDivisionSchema;
    },
  ...
  methods: {
    ...
    save() {
      ...
      this.$store
        .dispatch("saveDivisionData", item)
        .then(function(data) {
          try {
            self.close();
    ...
    edit() {
      ...
      let item = this.getSchema;
      this.$store
        .dispatch("editDivisionData", item)
        .then(function(data) {
          self.close();
          self.$emit("update:loadData", true);
          //self.$store.dispatch("fetchCenLsDistToRegData");
    ...
    clear() {
      ...
      this.$store.commit("UNSET_DIVISION_SCHEMA");
