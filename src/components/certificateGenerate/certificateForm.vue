...
export default {
  ...
  watch: {
    dialogStatus(val) {
      this.dialog = val;
      // seting email to username as usernm is missing in db
    },
    dialog(val) {
      if (!val) this.close();
    }
  ...
  computed: {
    getCerGenStudentList() {
      return this.$store.getters.getCerGenStudentDataList;
    },
    getSchema() {
      return this.$store.getters.getCertificateSchema;
    },
  ...
  methods: {
    addStudentList() {
      ...
      console.log("sssss", this.param);
      this.$store
        .dispatch("searchCertificateGenerateStudentData", this.searchParam)
        .then(data => {
    ...
    setParam() {
      ...
      this.param.reg_no = this.getCerGenStudentList[0].reg_no;
      this.param.name = this.getCerGenStudentList[0].name;
    },
    submit() {
      this.loading = true;
      this.setParam();
      this.$store
        .dispatch("submitGenerateCertificateData", this.param)
        .then(data => {
          // console.log("data---", data);
          this.loading = false;
        });
