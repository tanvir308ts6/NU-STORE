              ...
              <v-layout row wrap>
                ...
                <v-flex xs12>
                  <v-autocomplete
                    ...
                    v-model="getSchema.examiner_code"
                  ...
                  >
                    ...
                    <template v-slot:item="data">
                      <template v-if="typeof data.item !== 'object'">
                        <v-list-tile-content
                          v-text="data.item.examiner_name"
                        ></v-list-tile-content>
                      </template>
                      <template v-else>
                        <v-list-tile-content>
                          <v-list-tile-title
                            v-html="data.item.examiner_name"
                          ></v-list-tile-title>
                        </v-list-tile-content>
                      </template>
                    </template>
...
export default {
  mixins: [commonFun, commonGet, commonExamYear],
  $_veeValidate: {
    validator: "new"
  },
  components: {
  ...
  props: ["dialogStatus", "editedData", "indexEdited", "searchAnsScriptList"],
  data: () => ({
    dialog: false,
    editedStatus: false,
    dateMenu: false,
    dictionary: {
      custom: {
        ...
        reg_center_code: { required: () => "Select Regional Center." },
        examiner_code: { required: () => "Select Examiner." },
        exam_code: { required: () => "Select Exam." }
      }
    }
  }),
  created() {
  ...
  watch: {
    dialogStatus(val) {
      this.dialog = val;
      this.$store.commit("SET_EXAM_AS_COLL_FROM_REG_SCHEMA", this.editedData);
      ...
      // this.examiners = this.$store.getters.getExaminerItems;
    },
    dialog(val) {
      if (!val) this.close();
    }
  ...
  computed: {
    getSchema() {
      return this.$store.getters.getExamAsCollFromRegSchema;
    },
    getItemList() {
    ...
    formTitle() {
      return this.indexEdited === -1
        ? "Answer Script Collection From RC"
        : "Receive Answer Script Collection From RC";
    }
  },
  methods: {
    validationForm() {
    ...
    save() {
      ...
      this.$store
        .dispatch("saveExamAsCollFromRegData", item)
        .then(function() {
          try {
            self.close();
    ...
    edit() {
      ...
      let item = this.getSchema;
      this.$store
        .dispatch("editExamAsCollFromRegData", item)
        .then(function() {
          self.close();
          self.searchAnsScriptList();
        })
    ...
    clear() {
      ...
      this.$store.commit("UNSET_EXAM_AS_COLL_FROM_REG_SCHEMA");
