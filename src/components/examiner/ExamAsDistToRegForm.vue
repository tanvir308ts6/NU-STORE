          ...
          <v-card flat class="pr-3 scroll">
            <v-form>
              <v-layout row wrap>
                ...
                <v-flex xs12 v-if="examinerStatus">
                  <v-autocomplete
...
export default {
  ...
  watch: {
    dialogStatus(val) {
      ...
      if (userType == 1) {
        this.examinerStatus = true;
      }

      this.$store.commit("SET_EXAM_AS_DIST_TO_REG_SCHEMA", this.editedData);
  ...
  computed: {
    getSchema() {
      return this.$store.getters.getExamAsDistToRegSchema;
    },
    getItemList() {
      return this.$store.getters.getExamAsDistToRegDataList;
    },
    buttonTitle() {
  ...
  methods: {
    ...
    save() {
      ...
      if (user_type == "Examiner") {
        this.getSchema.examiner_code = this.getExaminerCode();
      }
      ...
      this.$store
        .dispatch("saveExamAsDistToRegData", item)
        .then(function() {
          try {
            self.close();
    ...
    edit() {
      ...
      let item = this.getSchema;
      this.$store
        .dispatch("editExamAsDistToRegData", item)
        .then(function() {
          self.close();
          self.searchAnswerScriptList();
        })
    ...
    clear() {
      ...
      this.$store.commit("UNSET_EXAM_AS_DIST_TO_REG_SCHEMA");
