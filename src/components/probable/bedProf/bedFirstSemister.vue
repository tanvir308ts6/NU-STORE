    ...
    >
      <v-card>
        <v-toolbar dark color="primary">
          <v-toolbar-title
            >Generate Probable For BEd 1st Semester</v-toolbar-title
          >
          <v-spacer></v-spacer>
          <v-toolbar-items>
                      ...
                      <template>
                        ...
                        <v-container fluid grid-list-xl>
                          <v-layout row wrap>
                            <v-flex md6>
                              <v-select
                                v-model="getSchema.first_sem_regu_ac_session"
                            ...
                            <v-flex md6>
                              <v-text-field
                                v-model="getSchema.elective_papers"
                                label="Elective Papers"
                                data-vv-name="elective_papers"
                              ></v-text-field>
                            </v-flex>
                          </v-layout>
                        </v-container>
                      ...
                      <template>
                        ...
                        <v-container fluid grid-list-xl>
                          <v-layout row wrap>
                            ...
                            <v-flex md4>
                              <v-text-field
                                v-model="getSchema.irregular_elective_papers"
                                label="Elective Papers"
                                data-vv-name="elective_papers"
                              ></v-text-field>
                            </v-flex>
                      ...
                      <template>
                        ...
                        <v-container fluid grid-list-xl>
                          <v-layout row wrap>
                            <v-flex md4>
                              <v-select
                                v-model="getSchema.irregular_fail_ac_session"
                                :items="getSchemaSession"
                                item-text="ac_session"
                                item-value="ac_session"
                                label="Select Academic Session"
                            ...
                            <v-flex md4>
                              <v-select
                                v-model="getSchema.irregular_fail_exam_year"
                                :items="getSchemaExamYear"
                                item-text="exam_year"
                                item-value="exam_year"
                                label="Select Exam Year"
                            ...
                            <v-flex md4>
                              <v-select
                                v-model="getSchema.irregular_fail_grade"
                                :items="grades"
                                label="Select Grade"
                                multiple
                              ></v-select>
                      ...
                      <template>
                        ...
                        <v-container grid-list-md text-xs-center>
                          <v-layout row wrap>
                            <v-flex md4>
                              <v-select
                                v-model="getSchema.improve_ment_pass_sess"
                                :items="getSchemaSession"
                                item-text="ac_session"
                                item-value="ac_session"
                                label="Select Academic Session"
                            ...
                            <v-flex md4>
                              <v-text-field
                                ...
                                label="No Of Paper"
                                data-vv-name="improve_pass_paper"
                              ></v-text-field>
                            </v-flex>
                            <v-flex md4>
                              <v-text-field
                                ...
                                label="CGPA"
                                data-vv-name="improve_pass_cgpa"
                              ></v-text-field>
                            </v-flex>
...
export default {
  ...
  computed: {
    getSchema() {
      return this.$store.getters.bedFirstSemester;
    },
    getSchemaSession() {
  ...
  methods: {
    save() {
      ...
      if (!isEmpty) {
        this.$store
          .dispatch("generateProbableBedProfFirstSem", data)
          .then(function(data) {
            try {
              self.close();
