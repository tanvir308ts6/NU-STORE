    ...
    <v-container fluid class="mt-2" v-if="getProbableList.length > 0">
      <v-card color="white">
        <v-layout row>
          ...
          <v-btn @click="downloadIncourseMarkPdf" color="primary" dark>
            Incourse Masks PDF
      ...
      >
        <template v-slot:items="props">
          <tr>
            ...
            <td class="text-xs teal lighten-4">
              <slot v-for="(paper, index) in props.item.papers">
                <v-chip :key="index" small color="green" text-color="white">
                  <!-- <v-avatar
                    right
                    class="green darken-4"
                    v-if="Number(paper.incourse_marks)"
                  >{{paper.incourse_marks}}</v-avatar>-->
                  <!-- {{` ${paper.paper_code}`}} -->
                  <span>{{ ` ${paper.paper_code}` }}</span>
                  <span v-if="Number(paper.incourse_marks)">{{
                    ` - ${paper.incourse_marks}`
                  }}</span>
                </v-chip>
                <!-- <span
                  :key="index"
                  v-if="paper.confirm_paper"
                >{{` ${paper.paper_code}`}}{{(paper.incourse_marks)?`-${paper.incourse_marks}`:''}},</span>-->
              </slot>
            </td>
            ...
            <td class="text-xs" v-if="props.item.college_user">
            ...
            <td class="text-xs">
              {{ props.item.teacher_confirmed_at | dateFormat("ll") }}
            </td>
            <td class="text-xs">
              <v-chip
                v-if="
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        {
          text: "Mark & Paper Code",
          align: "center",
          class: "teal lighten-4",
          value: "papers.paper_code"
        },
        { text: "Mobile", value: "student.mobile" },
        { text: "Candidate Type", value: "candidate_type_id" },
        { text: "Confirm By", value: "teacher_confirmed_by" },
        { text: "Confirm Date", value: "teacher_confirmed_at" },
  ...
  methods: {
    ...
    downloadExcel() {
      this.$store.dispatch("collegeProbableListDownloadExcel",{
        params: this.getProbableSchema,
        responseType: "blob"
      });
    },
    downloadIncourseMarkPdf() {
      this.getProbableSchema.incourse_status = true;
      this.$store
        .dispatch("collegeProbableListDownloadPdf", {
          params: this.getProbableSchema,
          responseType: "blob"
        })
        .then(response => {
          this.getProbableSchema.incourse_status = false;
        });
    },
    getDegreeCode() {
    ...
    setLimit() {
      ...
      this.getProbableSchema.teacher_confirm_status = 1;
    },
    searchList() {
      ...
      try {
        ...
        this.setLimit();
        this.$store
          .dispatch("getCollegeProbableList")
          .then(data => {
            this.loading = false;
