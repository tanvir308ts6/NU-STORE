        ...
        <v-layout row>
          <v-card flat>
            <v-card-text class="title"
              >Answer Script Collection From NU</v-card-text
            >
          </v-card>
          ...
          <v-spacer />
          <!-- <v-card-actions>
            <v-btn @click="newItem" color="primary" dark>Create New Ans Collection From NU</v-btn>
          </v-card-actions>-->
        </v-layout>
      </v-card>
      <v-data-table
        :loading="loading"
        :headers="headers"
        :items="getRegAsCollFromNuDataList"
        :search="search"
        class="elevation-1"
...
<script>
import formDialog from "@/components/regionalCenter/RegAsCollFromNuForm.vue";
...
export default {
  ...
  created() {
    ...
    //this.$store.dispatch("callRegAsCollFromNuData");
  },
  computed: {
    getRegAsCollFromNuDataList() {
      return this.$store.getters.getRegAsCollFromNuDataList;
    }
  },
  methods: {
    unSetEditedData() {
    ...
    searchAnswerScriptList() {
      ...
      this.setLimit();
      this.$store
        .dispatch("callRegAsCollFromNuData", this.searchParam)
        .then(data => {
          this.loading = false;
    ...
    editItem(item) {
      this.editedIndex = this.getRegAsCollFromNuDataList.indexOf(item);
    ...
    deleteItem(item) {
      ...
      this.$root
        ...
        .then(confirm => {
          if (confirm) {
            this.$store
              .dispatch("deleteRegAsCollFromNuItem", id)
              .then(function() {
