<template>
  <v-container fluid>
    ...
    <v-container fluid>
        ...
        <v-layout row>
          <v-card flat>
            <v-card-text class="title"
              >Answer Script Delivery To NU</v-card-text
            >
          </v-card>
          ...
          <v-card-actions>
            <v-btn @click="newItem" color="primary" dark
              >Create New Ans Delivery To NU</v-btn
            >
          </v-card-actions>
        </v-layout>
      </v-card>
      <v-data-table
        :headers="headers"
        :items="getRegAsDistToNuDataList"
        :search="search"
        class="elevation-1"
        :loading="loading"
...
<script>
import formDialog from "@/components/regionalCenter/RegAsDistToNuForm.vue";
...
export default {
  ...
  created() {
    ...
    //this.$store.dispatch("callRegAsDistToNuData");
  },
  computed: {
    getRegAsDistToNuDataList() {
  ...
  methods: {
    ...
    searchAnswerScriptList() {
      ...
      this.setLimit();
      this.$store
        .dispatch("searchRegAsDistToNuData", this.searchParam)
        .then(data => {
    ...
    editItem(item) {
      this.editedIndex = this.getRegAsDistToNuDataList.indexOf(item);
    ...
    deleteItem(item) {
      ...
      this.$root
        ...
        .then(confirm => {
          if (confirm) {
            this.$store
              .dispatch("deleteRegAsDistToNuItem", id)
              .then(function() {
                self.$store.dispatch("callRegAsDistToNuData");
              })
