        ...
        <v-layout row>
          <v-card flat>
            <v-card-text class="title"
              >Ans Script Collection From Center</v-card-text
            >
          </v-card>
          ...
          <v-spacer />
          <!-- <v-card-actions>
            <v-btn @click="newItem" color="primary" dark>Create New Ans Collection From Center</v-btn>
          </v-card-actions>-->
        </v-layout>
      </v-card>
      <v-data-table
        :headers="headers"
        :items="getRegAsCollFromCenDataList"
        :search="search"
        class="elevation-1"
        :loading="loading"
...
<script>
import formDialog from "@/components/regionalCenter/RegAsCollFromCenForm.vue";
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "MainQuantity", value: "main_quantity" },
  ...
  created() {
    ...
    //this.$store.dispatch("callRegAsCollFromCenData");
  },
  computed: {
    getRegAsCollFromCenDataList() {
  ...
  methods: {
    ...
    searchAnswerScriptList() {
      ...
      this.setLimit();
      this.$store
        .dispatch("searchRegAScollfromCenData", this.searchParam)
        .then(data => {
    ...
    editItem(item) {
      this.editedIndex = this.getRegAsCollFromCenDataList.indexOf(item);
    ...
    deleteItem(item) {
      ...
      this.$root
        ...
        .then(confirm => {
          if (confirm) {
            this.$store
              .dispatch("deleteRegAsCollFromCenItem", id)
              .then(function() {
                self.searchAnswerScriptList();
                //self.$store.dispatch("callRegAsCollFromCenData");
                // self.snackbar = true;
              })
