<template>
  <v-container fluid>
    ...
    <v-container fluid>
        ...
        <v-layout row>
          <v-card flat>
            <v-card-text class="title"
              >Blank Script Collection List</v-card-text
            >
          </v-card>
      ...
      <v-data-table
        :headers="headers"
        :items="getBlankScriptCollectList"
        :search="search"
        class="elevation-1"
        :rows-per-page-items="[
          25,
...
<script>
import bsCollectDialog from "@/components/regionalCenter/blankScriptCollectForm.vue";
export default {
  components: {
    bsCollectDialog
  },
  ...
  created() {
    this.$store.dispatch("callBlankScriptCollectData");
  },
  computed: {
    getBlankScriptCollectList() {
      return this.$store.getters.getBlankScriptCollectItem;
    },
    getMsg() {
      return this.$store.getters.getBlankScriptCollectMessage;
    }
  },
  methods: {
    editItem(item) {
      this.editedIndex = this.getBlankScriptCollectList.indexOf(item);
    ...
    deleteItem(item) {
      ...
      confirm("Are you sure you want to delete this item?") &&
        this.$store
          .dispatch("deleteBlankScriptCollectItem", index)
          .then(function(data) {
