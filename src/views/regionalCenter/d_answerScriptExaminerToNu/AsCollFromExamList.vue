        ...
        <v-layout row>
          <v-card flat>
            <v-card-text class="title"
              >Ans Script Collection From Examiner</v-card-text
            >
          </v-card>
          ...
          <v-spacer />
          <!-- <v-card-actions>
            <v-btn @click="newItem" color="primary" dark>Create New Ans Collection From Examiner</v-btn>
          </v-card-actions>-->
        </v-layout>
      </v-card>
      <v-data-table
        :loading="loading"
        :headers="headers"
        :items="getRegAsCollFromExamDataList"
        :search="search"
        class="elevation-1"
...
<script>
import formDialog from "@/components/regionalCenter/RegAsCollFromExamForm.vue";
...
export default {
  ...
  created() {
    ...
    //this.$store.dispatch("callRegAsCollFromExamData");
  },
  computed: {
    getRegAsCollFromExamDataList() {
  ...
  methods: {
    ...
    searchAnswerScriptList() {
      ...
      this.setLimit();
      this.$store
        .dispatch("callRegAsCollFromExamData", this.searchParam)
        .then(data => {
          this.loading = false;
    ...
    editItem(item) {
      this.editedIndex = this.getRegAsCollFromExamDataList.indexOf(item);
    ...
    deleteItem(item) {
      ...
      this.$root
        ...
        .then(confirm => {
          if (confirm) {
            this.$store
              .dispatch("deleteRegAsCollFromExamItem", id)
              .then(function() {
