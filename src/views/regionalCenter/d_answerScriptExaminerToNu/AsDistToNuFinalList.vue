<template>
  <v-container fluid>
    ...
    <v-container fluid>
        ...
        <v-layout row>
          <v-card flat>
            <v-card-text class="title"
              >Answer Script Delivery To NU Final</v-card-text
            >
          </v-card>
          ...
          <v-card-actions>
            <v-btn @click="newItem" color="primary" dark
              >Create New Ans Delivery To NU Final</v-btn
            >
          </v-card-actions>
        </v-layout>
      </v-card>
      <v-data-table
        :headers="headers"
        :items="getRegAsDistToNuFinalDataList"
        :search="search"
        class="elevation-1"
        :loading="loading"
...
<script>
import formDialog from "@/components/regionalCenter/RegAsDistToNuFinalForm.vue";
...
export default {
  ...
  created() {
    ...
    //this.$store.dispatch("callRegAsDistToNuFinalData");
  },
  computed: {
    getRegAsDistToNuFinalDataList() {
  ...
  methods: {
    ...
    searchAnswerScriptList() {
      ...
      this.setLimit();
      this.$store
        .dispatch("callRegAsDistToNuFinalData", this.searchParam)
        .then(data => {
    ...
    editItem(item) {
      this.editedIndex = this.getRegAsDistToNuFinalDataList.indexOf(item);
    ...
    deleteItem(item) {
      ...
      this.$root
        ...
        .then(confirm => {
          if (confirm) {
            this.$store
              .dispatch("deleteRegAsDistToNuFinalItem", id)
              .then(function() {
