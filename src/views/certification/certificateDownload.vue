              ...
              <v-layout row wrap align-center class="py-3">
                ...
                <v-flex xs12 sm12 md12 style="margin-top:40px;">
                  <v-layout justify-center row fill-height align-center>
                    <v-btn color="primary" large @click="searchCertificateList">Search</v-btn>
                  </v-layout>
                </v-flex>
              </v-layout>
            </v-card>
        ...
        <v-layout row>
          <v-card flat>
            <v-badge left overlap>
              <template v-slot:badge>{{ getCertificateItem.length }}</template>
              <v-card-text class="title">Certificate List</v-card-text>
            </v-badge>
          </v-card>
...
export default {
  ...
  created() {
    // all
    if (this.$store.getters.getCertificateItem.length == 0) {
      this.$store.dispatch("fetchCertificateData");
    }
  },
  computed: {
    //all data
    getCertificateItem() {
      return this.$store.getters.getCertificateItem;
    },
  },
  methods: {
    searchCertificateList() {
      ...
      this.$store
        .dispatch("searchCertificate", customSearchParam)
        .then(() => {});
    },
  },
};
