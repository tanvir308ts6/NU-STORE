<template>
  <v-container fluid>
    <formDialog :dialogStatus.sync="formDialogStatus" :indexEdited.sync="editedIndex"></formDialog>
        ...
        <v-layout row wrap align-center>
          ...
          <v-flex xs12 sm12 md12 class="mb-3">
            <v-layout justify-center row fill-height align-center>
              <v-btn outline round large :disabled="!searchParam.reg_no" @click="searchCertificateList()">
                <v-icon left dark></v-icon>Search
              </v-btn>
            </v-layout>
          </v-flex>
        </v-layout>
    ...
    <v-container fluid>
      <v-card color="white">
        <v-layout row>
          ...
          <v-spacer />
          <v-card-actions>
           <!--  <v-btn @click="newItem" color="primary" dark>Generate Certificate</v-btn> -->
          </v-card-actions>
        </v-layout>
      </v-card>
      <div v-if="openStat">
...
<script>
import formDialog from "@/components/certificateGenerate/certificateForm.vue";
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        // { text: "Print Date", value: "print_date", sortable: false },
        ...
        { text: "Generate Certificate", value: "degree", sortable: false },
  ...
  watch: {
    pagination() {
      this.searchCertificateList();
    }
  },
  computed: {
    getCertificateList() {
      return this.$store.getters.getCertificateDataList;
    }
  },
  methods: {
    unSetEditedData() {
    ...
    searchCertificateList() {
      ...
      this.openStat = true;
      this.setLimit();
      this.$store.dispatch("searchCertificate", this.searchParam).then(data => {
      ...
      this.setLimit();
      this.$store
        .dispatch("submitGenerateCertificateData", this.searchParam)
        .then(data => {
