    ...
    <div>
      ...
      <v-container fluid>
        ...
        <v-card color="white">
          <v-layout row>
            <v-card flat>
              <v-card-text class="title">Pending List</v-card-text>
            </v-card>
        ...
        >
          <template v-slot:items="props">
            ...
            <td class="text-xs-center">
              {{ props.item.payment_status == 1 ? "Paid" : "Unpaid" }}
            </td>
            <td class="text-xs-center text-capitalize">
...
<script>
import studentDetails from "@/components/service/certificateVerification/statusPendingDetails.vue";
...
export default {
  ...
  methods: {
    ...
    getCvListFromApi() {
      this.setLimit();
      this.loading = true;
      this.searchParam.status = "pending";
      this.$store
