    ...
    <div>
      ...
      <v-container fluid>
        ...
        <v-card color="white">
          <v-layout row>
            <v-card flat>
              <v-card-text class="title">Verified List</v-card-text>
            </v-card>
...
<script>
import studentDetails from "@/components/service/certificateVerification/StatusVerifiedDetails.vue";
...
export default {
  ...
  methods: {
    ...
    getCvListFromApi() {
      this.setLimit();
      this.loading = true;
      this.searchParam.status = "verified";
      this.$store
