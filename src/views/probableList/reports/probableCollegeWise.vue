            ...
            <v-card flat>
              <v-badge left overlap color="transparent">
                ...
                <v-card-text class="title"
                  >College & Subject Wise Probable List</v-card-text
                >
              </v-badge>
            </v-card>
        ...
        >
          <template v-slot:items="props">
            <table class="report-table">
              ...
              <tr>
                <td
                  :rowspan="props.item.students.length + 1"
                  width="35%"
                  class="text-xs-left"
                >
              ...
              <template v-for="(subject, i) in props.item.students">
                <tr>
                  <td width="55%" class="text-xs-left">
                    ({{ subject.subject_code }})
                    {{ subject.subject_name }}
                  </td>
                  <td class="text-xs-right">{{ subject.total_student }}</td>
                </tr>
              </template>
              <tr>
                ...
                <td class="text-xs-right font-weight-bold">
                  {{ totalStudent(props.item.students) }}
                </td>
              </tr>
            </table>
          </template>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      // {text: "Subject Code & Name", align: "center", sortable: false, value: "subject_code"},
      // {text: "Total Student", align: "center", sortable: false, value: "total_student"},
  ...
  computed: {
    getSchema() {
      return this.$store.getters.getProbableCollegeWiseDetails;
    },
    getColleges() {
  ...
  methods: {
    ...
    getDataFromApi() {
      ...
      this.loading = true;
      this.$store
        .dispatch("fetchProbableCollegeWiseDetailData", this.searchParam)
        .then(data => {
          this.loading = false;
    ...
    searchProbableCollegeWise() {
      ...
      this.$store
        .dispatch("fetchProbableCollegeWiseDetailData", self.searchParam)
        .then(data => {
          self.loading = false;
    ...
    exportReport() {
      ...
      this.$store.dispatch("exportProbableCollegeWise", this.searchParam);
    }
  },
  watch: {
    pagination() {
