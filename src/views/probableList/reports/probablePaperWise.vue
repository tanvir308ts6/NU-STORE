            ...
            <v-card flat>
              <v-badge left overlap color="transparent">
                ...
                <v-card-text class="title"
                  >Subject & Paper Wise Probable List</v-card-text
                >
              </v-badge>
            </v-card>
          ...
          <template v-slot:items="props">
            <table class="report-table">
              <tr v-if="props.index === 0">
                ...
                <th class="text-xs-left">Paper Code & Name</th>
                <th class="text-xs-left">Paper Type</th>
                <th class="text-xs-right">Total</th>
              </tr>
              <tr>
                <td
                  :rowspan="props.item.papers.length + 1"
                  width="25%"
                  class="text-xs-left"
                >
              ...
              <template v-for="(student, i) in props.item.papers">
                <tr>
                  <td width="50%" class="text-xs-left">
                    ({{ student.paper_code }})
                    {{ student.paper_name }}
                  </td>
                  <td class="text-xs-left">{{ student.paper_type }}</td>
                  <td class="text-xs-right">{{ student.total_student }}</td>
                </tr>
              </template>
              <tr>
                <td colspan="3" class="text-xs-right font-weight-bold">
                  Paper Total =
                </td>
                <td class="text-xs-right  font-weight-bold">
                  {{ totalStudent(props.item.papers) }}
                </td>
              </tr>
            </table>
          </template>
...
export default {
  ...
  computed: {
    getSchema() {
      return this.$store.getters.getProbablePaperWiseWiseDetails;
    },
    getSubjects() {
  ...
  methods: {
    ...
    getDataFromApi() {
      ...
      this.loading = true;
      this.$store
        .dispatch("fetchProbablePaperWiseDetailData", this.searchParam)
        .then((data) => {
          this.loading = false;
    ...
    searchProbablePaperWise() {
      this.loading = true;
      this.setLimit();
      let self = this;
      this.searchParam.status;
      this.$store
        .dispatch("fetchProbablePaperWiseDetailData", self.searchParam)
        .then((data) => {
          self.loading = false;
    ...
    exportReport() {
      ...
      this.$store.dispatch("exportProbablePaperWise", this.searchParam);
    },
  },
  watch: {
    pagination() {
