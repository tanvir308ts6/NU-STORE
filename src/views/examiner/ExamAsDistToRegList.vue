  ...
  <v-container fluid>
    ...
    <v-container fluid>
      <v-card color="white">
        <v-layout row>
          <v-card flat>
            <v-card-text class="title">Ans Script Delivery To RC</v-card-text>
          </v-card>
      ...
      <!-- :rows-per-page-items="[ 25, 50, 100, { text:
      '$vuetify.dataIterator.rowsPerPageAll', value: -1 } ]"-->
      <v-data-table
        :headers="headers"
        :items="getExamAsDistToRegDataList"
        :search="search"
        class="elevation-1"
        :loading="loading"
...
<script>
import formDialog from '@/components/examiner/ExamAsDistToRegForm.vue';
...
export default {
  ...
  created() {
    ...
    if (this.ifExaminerUser) {
      ...
      //or //this.searchParam.center_code =this.getAuthUser.center.center_code;
    }

    this.$store.commit('UNSET_FORMNOTIFY');
    //this.$store.dispatch("callExamAsDistToRegData");
  },
  computed: {
    getExamAsDistToRegDataList() {
  ...
  methods: {
    ...
    searchAnswerScriptList() {
      ...
      this.setLimit();
      this.$store
        .dispatch('callExamAsDistToRegData', this.searchParam)
        .then((data) => {
          this.loading = false;
    ...
    newItem() {
      ...
      this.editedItem.exam_code = this.getLoggedInExam.code; // this line
      this.editedItem.year = this.getLoggedInYear.year; // this line
    },
    editItem(item) {
      this.editedIndex = this.getExamAsDistToRegDataList.indexOf(item);
    ...
    deleteItem(item) {
      ...
      this.$root
        ...
        .then((confirm) => {
          if (confirm) {
            this.$store
              .dispatch('deleteExamAsDistToRegItem', id)
              .then(function() {
