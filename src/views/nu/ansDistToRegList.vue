        ...
        <v-layout row>
          <v-card flat>
            <v-card-text class="title"
              >Answer Script Delivery To Regional Center</v-card-text
            >
          </v-card>
...
<script>
import formDialog from "@/components/nu/ansDistToRegForm.vue";
...
export default {
  ...
  created() {
    ...
    //this.$store.dispatch("fetchNuAsDistToRCData");
  },
  watch: {
    pagination() {
  ...
  methods: {
    ...
    searchAnswerScriptList() {
      ...
      this.setLimit();
      this.$store
        .dispatch("fetchNuAsDistToRCData", this.searchParam)
        .then(data => {
          this.loading = false;
    ...
    editItem(item) {
      this.editedIndex = this.getNuAsDistToRCDataList.indexOf(item);
    ...
    deleteItem(item) {
      ...
      this.$root
        ...
        .then(confirm => {
          if (confirm) {
            this.$store
              .dispatch("deleteNuAsDistToRCItem", id)
              .then(function() {
