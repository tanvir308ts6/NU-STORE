  ...
  <v-container fluid>
    ...
    <v-container fluid>
        ...
        <v-layout row>
          <v-card flat>
            <v-card-text class="title"
              >Answer Script Collection From Regional Center</v-card-text
            >
          </v-card>
      ...
      <v-data-table
        :loading="loading"
        :headers="headers"
        :items="getNuAsCollFromRCDataList"
        :search="search"
        class="elevation-1"
...
<script>
import formDialog from "@/components/nu/ansCollFromRcForm.vue";
...
export default {
  ...
  created() {
    ...
    //this.$store.dispatch("fetchNuAnsCollFromRCData");
  },
  computed: {
    getNuAsCollFromRCDataList() {
      return this.$store.getters.getNuAsCollFromRCDataList;
    }
  },
  watch: {
    pagination() {
  ...
  methods: {
    ...
    searchAnswerScriptList() {
      ...
      this.setLimit();
      this.$store
        .dispatch("fetchNuAnsCollFromRCData", this.searchParam)
        .then(data => {
          this.loading = false;
    ...
    editItem(item) {
      this.editedIndex = this.getNuAsCollFromRCDataList.indexOf(item);
    ...
    deleteItem(item) {
      ...
      this.$root
        ...
        .then(confirm => {
          if (confirm) {
            this.$store
              .dispatch("deleteNuAsCollFromRCItem", id)
              .then(function() {
                self.searchAnswerScriptList();
                //self.$store.dispatch("fetchNuAnsCollFromRCData");
              })
