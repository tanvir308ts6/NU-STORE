<template>
  <div>
    ...
    <v-flex xs9 sm9 md9 layout>
      <div class="mx-4">
        <span class="headline" v-if="student"
          >{{ student.name }} (Reg No: {{ student.reg_no }})</span
        >
      </div>
    </v-flex>
...
import navDrawer from "@/layouts/navDrawerForStudentPanel.vue";
...
export default {
  ...
  methods: {
    getStudent() {
      let self = this;
      Axios.get(self.getBaseUrl + "/api/student/std_profile").then(response => {
